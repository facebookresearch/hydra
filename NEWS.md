...
- Fix  all integer/float choice from spec in the Nevergrad plugin
...
- All plugins  are updated to use TargetConf. Be sure to update all your Hydra plugins ([#882](https://github.com/facebookresearch/hydra/issues/882))
- Instantiated objects  are no longer have a params node in the conf. keys like hydra.launcher.params.foo are changed to hydra.launcher.foo. Update your configs and overrides if you were overriding such parameters. ([#882](https://github.com/facebookresearch/hydra/issues/882))
...
- Prints error when command line override  it looks like a sweep but --multirun is not specified ([#712](https://github.com/facebookresearch/hydra/issues/712))
...
- Fix interpolations in overrides  is used to evaluate lazily ([#725](https://github.com/facebookresearch/hydra/issues/725))
...
- Add  and support for overriding package from command line and defaults list ([#235](https://github.com/facebookresearch/hydra/issues/235))
- Config source is now abstracted,  is used by allowing additional config sources to be used ([#257](https://github.com/facebookresearch/hydra/issues/257))
...
- Improves performance of plugin discovery and instantiation ([#489](https://github.com/facebookresearch/hydra/issues/489))
...
- User plugins it should be modified to not import twice during plugin discovery. see issue for details. ([#482](https://github.com/facebookresearch/hydra/issues/482))
...
- Allows  configuring override_dirname via hydra.job.config.override_dirname to exclude specific keys or change separator characters (#95)
...
- Multi change diff:
    ...
    - Added selective plugin testing  and  using env, for example PLUGINS=fairtask would only test fairtask. (#87)
