        ...
        if isinstance(dd, InputDefault) and dd.is_interpolation():
            try:
                ...
                if not parent.primary:
                    # Defaults List interpolations outside of the primary config are not supported.
                    # We try to resolve anyway because custom resolvers should mostly work.
                    path = parent.get_config_path()
                    raise ConfigCompositionException(
                        f"In '{path}': Defaults List interpolation is only supported in the primary config : {e}"
                    )
                else:
                    raise
