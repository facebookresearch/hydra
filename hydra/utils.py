...
def instantiate(
    ...
    **kwargs: Any,
    ...
    # Make shallow copy if config is a standard dictionary
    ...
    if isinstance(config, dict) or OmegaConf.is_dict(config):
        parse = config.pop(Keys.PARSE) if Keys.PARSE in config else parse
        parse = ParseMode(parse if parse is not None else ParseMode.NONE)

        convert = config.pop(Keys.CONVERT) if Keys.CONVERT in config else convert
        convert = ConvertMode(convert if convert is not None else ConvertMode.NONE)

        recursive = (
            config.pop(Keys.RECURSIVE) if Keys.RECURSIVE in config else recursive
        )
    ...
    if isinstance(config, dict):
        target = (
            _resolve_target(config.pop(Keys.TARGET)) if _is_target(config) else None
        )
        if recursive:
            config = {
                key: instantiate(
        ...
        return _call_target(target, *args, **config) if target else config

    if OmegaConf.is_dict(config):
        if _is_target(config):
            target = _resolve_target(config.pop(Keys.TARGET))
            if recursive:
                config = {
                    key: instantiate(
                        value, parse=parse, convert=convert, recursive=recursive
            ...
            return _call_target(target, *args, **config)
