...
Tagging the interval with `log` will tell Ax to sample in log space (e.g. `python example/banana.py -m banana.x=tag(log, interval(1, 1000))`). When specifying a range sweep in a config file, include `log_scale: true` to achieve the same.
