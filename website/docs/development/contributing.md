...
`yamllint` will forbid the use of document start marker (`---`). When running locally, `yamllint` will run on all the yaml files, including the ones generated by hydra runs. We ignore the yaml files present in any `.hydra` directory.
