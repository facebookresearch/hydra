...
* `nox -s "test_plugins-3.10"` will test plugins on Python 3.10.
* `nox -s "test_plugins-3.14"` will test plugins on Python 3.14.

The `noxfile.py` is checking some environment variables to decide what to run. For example,
to test a single plugin:
```shell {4}
$ PLUGINS=hydra_colorlog nox -s test_plugins-3.10
Operating system        :       Linux
NOX_PYTHON_VERSIONS     :       ['3.10', '3.11', '3.12', '3.13', '3.14']
PLUGINS                 :       ['hydra_colorlog']
SKIP_CORE_TESTS         :       False
FIX                     :       False
VERBOSE                 :       0
INSTALL_EDITABLE_MODE   :       0
nox > Running session test_plugins-3.10
...
```
