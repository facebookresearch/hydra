...
application still runs on Hydra 1.3. Applications already using
`version_base=None`, `"1.2"`, or `"1.3"` already use the surviving behavior.

After upgrading to Hydra 1.4, remove `version_base`. Hydra 1.4 rejects `"1.1"`
and older values because their behavior no longer exists, and warns on all
other explicit values. Hydra 1.5 removes the parameter completely.

### Behavior removed with version_base="1.1"

Moving from the 1.1 behavior to the surviving behavior makes these changes:
...
- The default config search path changes from the calling directory to no
  ...
  [Changes to @hydra.main() and hydra.initialize()](../1.1_to_1.2/hydra_main_config_path.md).
- `hydra.job.chdir` defaults to `False`. Set it explicitly if the application
  must run from Hydra's output directory. See
  [Changes to the job working directory](../1.1_to_1.2/changes_to_job_working_dir.md).
...
- Defaults List entries that replace Hydra config groups need the `override`
  keyword. See
  [Defaults List overrides](../1.0_to_1.1/defaults_list_override.md).
- Replace indexed Defaults List interpolations such as
  `${defaults.0.dataset}`. See
  [Defaults List interpolation](../1.0_to_1.1/defaults_list_interpolation_changes.md).
