...
class SubmititLauncherDefaults(SearchPathPlugin):
    def manipulate_search_path(self, search_path: ConfigSearchPath) -> None:
        search_path.append("hydra-submitit-launcher", "pkg://hydra_plugins.submitit.conf")
...
class SubmititLauncher(Launcher):
    ...
    def setup(self, config: DictConfig, config_loader: ConfigLoader, task_function: TaskFunction):
        self.config = config
        self.config_loader = config_loader
        self.task_function = task_function

    def __call__(self, sweep_overrides: List[str], job_dir_key: str, job_num: int,
        ...
        return run_job(config=sweep_config, task_function=self.task_function, job_dir_key=job_dir_key,
    ...
    def launch(self, job_overrides: Sequence[Sequence[str]], initial_job_idx: int) -> Sequence[JobReturn]:
        ...
        assert num_jobs > 0
        with open_dict(self.config.hydra.job) as job:
        ...
        if self.queue == "auto":
            ...
            executor = submitit.AutoExecutor(folder=self.folder, max_num_timeout=max_num_timeout, )
            ...
            executor = submitit.SlurmExecutor(folder=self.folder, max_num_timeout=max_num_timeout)
        ...
        for job_num in range(num_jobs):
            ...
            log.info("\t#{} : {}".format(job_num, " ".join(filter_overrides(sweep_override)), ))
            params.append((sweep_override, "hydra.sweep.dir", job_num, Singleton.get_state(),))
