...
@pytest.mark.parametrize('task_config, dot_hydra, overrides, expected_dir', [
    ...
    (OmegaConf.create({'hydra': {'run': {'dir': 'foo'}}}), {}, ['hydra.run.dir=boom'], 'boom'),

    # hydra config override
    # test with task config override
    ({}, OmegaConf.create({'hydra': {'run': {'dir': 'foo'}}}), [], 'foo'),
    ({}, OmegaConf.create({'hydra': {'run': {'dir': 'abc'}}}), ['hydra.run.dir=efg'], 'efg'),
    # test with both task config and command line override
    ({}, OmegaConf.create({'hydra': {'run': {'dir': 'abc'}}}), ['hydra.run.dir=efg'], 'efg'),
    (OmegaConf.create({'hydra': {'run': {'dir': 'abc'}}}),
     OmegaConf.create({'hydra': {'run': {'dir': 'def'}}}),
     ['hydra.run.dir=boom'], 'boom'),

])
@pytest.mark.parametrize('task_launcher_cfg, extra_flags, plugin_module', [
    (None, [], None),
    # (create_fairtask_launcher_local_config(), ['-s'], 'hydra_plugins.fairtask'),
    # TODO: re-enable after submitit local queue is fixed
    # pytest.param(
...
def test_custom_workdir(tmpdir,
                        task_config,
                        dot_hydra,
                        overrides,
                        expected_dir,
    ...
    verify_plugin(plugin_module)
    overrides = extra_flags + overrides
    cfg = OmegaConf.merge(task_launcher_cfg or OmegaConf.create(),
                          task_config or OmegaConf.create())

    expected_dir1 = tmpdir / expected_dir
    integration_test(tmpdir,
                     task_config=cfg,
                     hydra_config=dot_hydra,
                     overrides=overrides,
                     prints="os.getcwd()",
                     expected_outputs=str(expected_dir1))

# TODO:
# Add tests for customizing workdir : split tests of launchers workdir becahse it behaved
# differently
